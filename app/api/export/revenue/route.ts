...
export const GET = withAuth(
  async (req: NextRequest) => {
    try {
      ...
      const buffer = await exportService.exportRevenue(query.from, query.to);
      const filename = `revenue_${query.from.toISOString().split("T")[0]}_${query.to.toISOString().split("T")[0]}.xlsx`;
